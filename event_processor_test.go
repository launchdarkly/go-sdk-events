...
func TestEventMetricsRecordsDroppedEventsFromOutbox(t *testing.T) {
	// Use RecordRawEvent to avoid side-effect events (index, summary) and get precise control.
	// Use a large enough capacity that the inbox channel (also sized to capacity) won't drop,
	// then fill the outbox to capacity and send one more to trigger a drop.
	metrics := &mockEventMetrics{}
	config := basicConfigWithoutPrivateAttrs()
	config.Capacity = 3
	config.EventMetrics = metrics

	ep, es := createEventProcessorAndSender(config)
	defer ep.Close()

	// Send events one at a time, draining the inbox between each to ensure all reach the outbox.
	for i := 0; i < 4; i++ {
		ep.RecordRawEvent(json.RawMessage(fmt.Sprintf(`{"kind":"custom","key":"e%d"}`, i)))
		ep.waitUntilInactive()
	}
	ep.Flush()
	ep.waitUntilInactive()

	// Drain the sent events
	for {
		if _, ok := es.tryAwaitEvent(); !ok {
			break
		}
	}

	// 3 events fill the outbox, 4th is dropped
	assert.Equal(t, 1, metrics.droppedCount)
}

func TestEventMetricsNilDoesNotPanic(t *testing.T) {
	// Verify that not setting EventMetrics doesn't cause panics when events are dropped
	config := basicConfigWithoutPrivateAttrs()
	config.Capacity = 1
	...
	ep.RecordRawEvent(json.RawMessage(`{"kind":"custom","key":"e1"}`))
	ep.waitUntilInactive()
	ep.RecordRawEvent(json.RawMessage(`{"kind":"custom","key":"e2"}`))
	ep.waitUntilInactive()
	ep.Flush()
	ep.waitUntilInactive()

	// Drain events - we just care that it didn't panic
	for {
		if _, ok := es.tryAwaitEvent(); !ok {
			break
		}
...
func TestEventMetricsRecordsMultipleDrops(t *testing.T) {
	// Verify that the dropped count accumulates across multiple overflow events.
	metrics := &mockEventMetrics{}
	config := basicConfigWithoutPrivateAttrs()
	config.Capacity = 2
	config.EventMetrics = metrics
	...
	// Fill outbox to capacity, then send 3 more events that should all be dropped
	for i := 0; i < 5; i++ {
		ep.RecordRawEvent(json.RawMessage(fmt.Sprintf(`{"kind":"custom","key":"e%d"}`, i)))
		ep.waitUntilInactive()
	}
	...
	// 2 events fill the outbox, 3 are dropped
	assert.Equal(t, 3, metrics.droppedCount)
}

func TestEventMetricsRecordsEventsSent(t *testing.T) {
	// Verify that successfully sent events are recorded via EventMetrics.
	metrics := &mockEventMetrics{}
	config := basicConfigWithoutPrivateAttrs()
	config.EventMetrics = metrics
	...
	// 2 raw events + 1 summary event = 3 events sent
	// (the summary is auto-generated by the event processor on flush)
	assert.Equal(t, 2, metrics.sentCount)
	assert.Greater(t, metrics.bytesSent, 0)
}

func TestEventMetricsRecordsPendingEvents(t *testing.T) {
	metrics := &mockEventMetrics{}
	config := basicConfigWithoutPrivateAttrs()
	config.EventMetrics = metrics
	...
	// Add 3 events one at a time and verify depth increases
	ep.RecordRawEvent(json.RawMessage(`{"kind":"custom","key":"e1"}`))
	ep.waitUntilInactive()
	assert.Equal(t, 1, metrics.lastPendingEvents)

	ep.RecordRawEvent(json.RawMessage(`{"kind":"custom","key":"e2"}`))
	ep.waitUntilInactive()
	assert.Equal(t, 2, metrics.lastPendingEvents)

	ep.RecordRawEvent(json.RawMessage(`{"kind":"custom","key":"e3"}`))
	ep.waitUntilInactive()
	assert.Equal(t, 3, metrics.lastPendingEvents)

	// After flush, depth should be 0
	ep.Flush()
	ep.waitUntilInactive()
	...
	assert.Equal(t, 0, metrics.lastPendingEvents)
}

func TestEventMetricsRecordsEventsFailedSend(t *testing.T) {
	metrics := &mockEventMetrics{}
	config := basicConfigWithoutPrivateAttrs()
	config.EventMetrics = metrics
	...
	// Configure the mock sender to return failure
	es.lock.Lock()
	es.result = EventSenderResult{Success: false}
	es.lock.Unlock()
	...
	assert.Equal(t, 2, metrics.failedSendCount)
}
