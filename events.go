...
// of event production and is not actually a property of the context, the Context builder does
// not have a way to embed that information in the context so it is added here separately.
// This requirement is specific to ld-relay and its use of go-sdk-events outside of the SDK; it
// does not apply to regular usage of the Go SDK or any other SDK.
type EventContext struct {
	Context ldcontext.Context
	// AlreadyFilteredAttributes is a list of private attribute names that were already removed
	// from the context before generating the event. If this is non-nil, the usual attribute
	// filtering logic will be skipped and this list will be passed on unchanged in the
	// privateAttrs property of the output context.
	AlreadyFilteredAttributes []string
}

// Context is a convenience function to convert ldcontext.Context to EventContext.
func Context(baseContext ldcontext.Context) EventContext {
	return EventContext{baseContext, nil}
}

// GetKey is a shortcut for getting the key of the wrapped context.
func (ec EventContext) GetKey() string {
	return ec.Context.Key()
}
